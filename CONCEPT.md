...
- Unlocked areas remain accessible
...
- Used for:
  ...
  3. Can be saved between stages

**Score:**
- Total essence collected = score
- Risk/reward: collect everything vs. stay safe
- Theoretical max score exists (perfect run)
- Accumulates across all areas/stages in single run

### Power-Ups (Temporary)

**Spawn Mechanics:**
- Random spawning throughout stage (not enemy drops)
- Unpredictable timing
- Count and probability determined by stage difficulty
...
- Stage-duration: Lasts until stage ends
- Some stackable (armor), others not (bullet multiplier)

*All lost when stage completes or player dies*

### Shop & Upgrades (Permanent until death)

**Shop Access:**
- Optional after each stage (can skip)
...
**Acquisition:**
- Awarded at final stage of each area
- Choose 1 of 3 random options from area's pool
...
- **Area**: Themed environment within world (e.g., Forest, Desert, Tundra)
- **Stage**: Individual playable level (e.g., Forest Stage 1, Forest Stage 2)

### Earth World (Early Access)

**3 Areas:**

1. **Forest Area** (Green) - 5 stages (short)
2. **Desert Area** (Yellow) - 10 stages (medium)
3. **Tundra/Ice Area** (White) - 15 stages (long)

**Progression:**
- Sequential unlock: Complete final stage of area → next area unlocks
- Once unlocked, choose area entry order freely
- Unique enemy sets per area
- Escalating difficulty and rewards
- Escalating shop prices
- Unified skill tree (shared across all areas)

**DLC/World Strategy:**
- 3-5 areas per world
- First DLC: Additional Earth areas
- Future Worlds: Moon World, Mars World, other planets
...
1. "Stage Finished" banner with 3-second countdown
2. Exit portal appears
3. Essence continues to decay
4. Collect remaining essence before timer expires
5. Auto-teleport to next stage

**Difficulty Scaling:**
- Linear scaling within each area
- Scales by: wave count, enemy composition, spawn rate

**Obstacles (2 types per area):**

| Area | Solid (blocks all) | Flat (blocks movement only) |
|-------|-------------------|----------------------------|
...
**End of Area:**
- Stats summary: essence collected, stages cleared, power-ups used
- Essence and coins carry over to next area
- Options:
  1. Enter next area (if unlocked)
  2. Re-run current area
  3. Return to main menu
- If no next area: End game (credits/menu)

**Death:**
- Run ends immediately
- All progress lost (upgrades, currency, items, equipped relics)
- Persistent: unlocked relics, unlocked areas
- Stats summary (same as area completion)
- Return to main menu
...
- **Area-Specific Pools**: Unique stage tracks per area
  - Forest: Nature-themed
  - Desert: Arid/wind-themed
  - Tundra: Icy/cold-themed

### Sound Effects
- Unified across game (action-specific, not area-specific)
...
### Boss Encounters
- Final stage of each area features boss
